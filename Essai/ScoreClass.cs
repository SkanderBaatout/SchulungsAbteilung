...
namespace Essai
{
    internal class ScoreClass
    {
        ...
        public bool updateScore(int stid, double scor, string desc)
        {
            MySqlCommand command = new MySqlCommand("UPDATE `score` SET `Score`=@sco,`Description`=@desc WHERE `EmployeeId`=@stid", connect.GetConnection);
            //@stid,@sco,@desc
            command.Parameters.Add("@stid", MySqlDbType.Int32).Value = stid;
            command.Parameters.Add("@sco", MySqlDbType.Double).Value = scor;
            command.Parameters.Add("@desc", MySqlDbType.VarChar).Value = desc;
